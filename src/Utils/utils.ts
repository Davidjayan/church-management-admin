...
export const buildHtml = (
  ...
  totalsColumnNames.forEach((col) => {
    if (col != 'ChurchOffering' && col != 'GrandTotal') {
  ...
  const tableOnehtmlHeaderTh = `<thead><tr><th colspan='11'>JEHOVAH NISSI AG CHURCH ACCOUNTS REPORT</th></tr><tr><th colspan='11'>${toDate ? `REPORT FROM ${moment(fromDate).format('DD/MM/YYYY')} TO ${moment(toDate).format('DD/MM/YYYY')}`:`REPORT ON ${moment(fromDate).format('DD/MM/YYYY')}`} </th>
     </tr><tr>
      ${headerItems}
     </tr></thead>`
  ...
  const tableOne = `<table id="t1">${tableOnehtmlHeaderTh}<tbody>${tableOnehtmlBodyTd}<tr>${totalsitems}</tr></tbody></table>`
